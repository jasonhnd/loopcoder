...
### Stabilization Gate (active — before further feature development)
...
fixture; it is not the v0.9 development controller. Self-bootstrap, roadmap
compilation, active-slice mutation, automatic issue creation, and provider
dispatch by LoopCoder are explicitly outside this development program.

The product sequence is user-first:

1. establish compact local authority outside every customer repository;
2. make one local attempt visible, bounded, stoppable, and recoverable;
3. make one explicit issue/provider/model run reach one PR reliably;
4. consolidate installed providers, models, quota windows, and deterministic
   routing without overriding user pins;
5. add explicit bounded workflows and contained sub-agents; and
6. migrate, delete superseded machinery, document, qualify, and release.

All work remains in v0.9.0. The checkpoints below are evidence boundaries, not
versions, prereleases, packages, or permission to move forward automatically.

| Phase | Outcome | Draft issues | Checkpoint evidence |
| --- | --- | ---: | --- |
| P0 - Development foundation | Platform, CI, threat, configuration, and fixture boundaries stop repeating v0.8 process failures. | 5 | Ordinary-development policy enforced; no self-bootstrap path used. |
| P1 - Local authority | Machine/project stores, stable project identity, append-only events, cursors, and restart integrity. | 8 | No runtime state enters a customer repo; event replay is deterministic. |
| P2 - Visible safe runtime | Supervised process tree, resource admission, universal UI reports, attention, follow stream, and honest delivery. | 20 | Required reports render through the public protocol; a 12-minute silent worker remains visible and bounded without model polling. |
| P3 - Direct run | `loopcoder run` takes one issue and an explicit route through one worker, commit, push, PR, CI, verifier, and human gate. | 16 | Terminal and generic-bridge canaries prove the first usable source build. |
| P4 - Provider plane and router | Codex, Claude, Gemini/Antigravity, and Grok discovery, invocation, quota, accounting, and explainable routing. | 27 | User pins never change silently; automatic choices are evidence-backed and replayable. |
...
- [`v0.9.0-product-charter.md`](docs/architecture/v0.9.0-product-charter.md)
- [`v0.9.0-external-inspiration-ledger.md`](docs/architecture/v0.9.0-external-inspiration-ledger.md)
- [`v0.9.0-operational-slos.md`](docs/architecture/v0.9.0-operational-slos.md)
- [`v0.9.0-domain-state-contract.md`](docs/architecture/v0.9.0-domain-state-contract.md)
- [`v0.9.0-storage-migration-contract.md`](docs/architecture/v0.9.0-storage-migration-contract.md)
- [`v0.9.0-ui-report-protocol.md`](docs/architecture/v0.9.0-ui-report-protocol.md)

The phases group ownership. Implementation advances through dependency-backed
capability gates: foundation, durable authority, universal visibility, visible
direct run, hardened multi-UI runtime, provider/router, workflow, and release.
...
## Superseded v0.9.0 self-bootstrap design (historical, non-authoritative)

The following design record explains earlier architecture decisions. Its
compiler lifecycle, Gate A-H controller model, issue counts, and historical
slice markers are superseded. Every former `planned-*`, `doc`, `code`, and
`shipped-*` marker in this section is deliberately renamed `historical-*` so an
old compiler cannot interpret it as active work. This section must not create
work or override the ordinary-development roadmap linked above.

### Owned Go-native development orchestration kernel <!-- lc:u=lc-b0e1d07c3d4c -->
...
- historical-shipped-doc: **R0.1 Product charter** - define user, default one-worker workflow, <!-- lc:u=lc-7304f225c290 -->
  ownership boundaries, GitHub/local/provider authorities, default/advanced
  modes, and explicit non-goals.
- historical-shipped-doc: **R0.2 External inspiration ledger** - pin Gas City, Beads, Plexus, and Orca; <!-- lc:u=lc-c785349ec276 -->
  record revisions and licenses,
  adopted concepts, rejected assumptions, rewrite-only policy, and provenance
  review checklist.
- historical-planned-doc: **R0.3 v0.8 disposition map** - map every current top-level subsystem to
  keep, consolidate, compatibility-only, or remove; name the owning v0.9
  component and deletion gate.
- historical-shipped-doc: **R0.4 Operational SLOs** - define the report-delivery contract:
  start,
...
- historical-planned-code: **R0.5 Baseline measurement command** - report current binary startup,
...
- historical-shipped-doc: **R1.1 Domain/state contract** - define Project, WorkItem, Dependency,
  Job, Attempt, Provider, Model, Gate, Event, legal transitions, idempotency, and
  authority.
- historical-shipped-doc: **R1.2 Storage/migration contract** - define the compact schema,
  transaction boundaries, event cursor, v0.8 read-only import, backup, abort,
  rollback, and file permissions.
- historical-shipped-code: **R1.3 Store open/schema foundation** - merged by #1089; create/open, schema version,
  owner-only permissions, integrity check, and close behavior without domain
  writes.
- historical-planned-code: **R1.4 Append-only event and cursor API** - idempotency key,
  monotonic sequence, ordered replay, compact current-status projection, and
  projection/host checkpoint primitives.
- historical-planned-code: **R1.5 SQLite contention/restart policy** - WAL, busy classification,
  bounded retry, transaction rollback, abrupt-restart, and two-connection tests.
- historical-planned-code: **R1.6 Project identity projection** - GitHub identity, path aliases,
  repository privacy, same-name isolation, and multi-repository tests.
- historical-planned-code: **R1.7 Job/attempt state machine** - guarded transitions and atomic event
...
- historical-planned-doc: **R2.1 Work Graph contract** - one-node default, edge semantics, graph
  bounds, ready set, claim/close guards, GitHub projection, and compaction.
- historical-planned-code: **R2.2 Graph validation and ready set** - cycle/root/depth/fan-out checks
  and deterministic ordering as pure Go logic.
- historical-planned-code: **R2.3 Atomic claim and guarded close** - compare-and-set transitions and
  event append in one SQLite transaction with two-connection contention tests.
- historical-planned-code: **R2.4 GitHub issue projection** - import/reconcile the root issue and
  terminal PR/check outcome without bidirectional task-database sync.
- historical-planned-code: **R2.5 Terminal compaction** - bounded summary retaining final state,
...
- historical-planned-doc: **R3.1 Provider adapter protocol** - versioned interface, timeout,
  redaction, auth ownership, quota source authority, model capability, and
  conformance fixtures.
- historical-planned-code: **R3.2 Adapter conformance harness** - common fake CLI/API fixtures,
  deadlines, redaction assertions, unknown/stale semantics, and contract golden
  files without a real provider call.
- historical-planned-code: **R3.3 Snapshot/cache core** - typed installation, auth, model, quota,
  and health observations with freshness/confidence and no credentials.
- historical-planned-code: **R3.4 Health and cooldown core** - classified failures, success reset,
  bounded exponential cooldown, persistence, and per-provider/model isolation.
- historical-planned-code: **R3.5 Codex discovery/auth/catalog adapter** - executable discovery,
  non-secret auth readiness, models, capabilities, and fixtures.
- historical-planned-code: **R3.6 Codex quota adapter** - authoritative short/weekly windows when
  available, reset timestamps, caching, redaction, and honest unknown fallback.
- historical-planned-code: **R3.7 Codex invocation/outcome adapter** - argv/environment, bounded
  output observations, actual model/effort evidence, and typed outcome mapping.
- historical-planned-code: **R3.8 Claude discovery/auth/catalog adapter** - executable discovery,
  non-secret auth readiness, models, capabilities, and fixtures.
- historical-planned-code: **R3.9 Claude quota adapter** - authoritative five-hour/seven-day
  windows when available, reset timestamps, redaction, and unknown fallback.
- historical-planned-code: **R3.10 Claude invocation/outcome adapter** - argv/environment,
  bounded observations, actual model/effort evidence, and typed outcomes.
- historical-planned-code: **R3.11 Antigravity/Gemini discovery/auth/catalog adapter** - `agy`
  discovery, non-secret readiness, dynamic models/capabilities, and fixtures.
- historical-planned-code: **R3.12 Antigravity/Gemini quota adapter** - bounded supported sources;
  unavailable or unauthoritative windows remain `unknown`.
- historical-planned-code: **R3.13 Antigravity/Gemini invocation/outcome adapter** - workspace
  pinning, argv/environment, bounded observations, and typed outcomes.
- historical-planned-code: **R3.14 Grok discovery/auth/catalog adapter** - executable discovery,
  non-secret readiness, dynamic models/capabilities, and no fabricated default.
- historical-planned-code: **R3.15 Grok quota adapter** - bounded supported sources; unavailable or
  unauthoritative windows remain `unknown`.
- historical-planned-code: **R3.16 Grok invocation/outcome adapter** - argv/environment, bounded
  observations, actual model/effort evidence, and typed outcomes.
- historical-planned-code: **R3.17 Future-provider registration kit** - fixtures and a small adapter
...
- historical-planned-doc: **R4.1 Routing policy** - hard eligibility, Luna/Tera/Soul mapping,
...
- historical-planned-code: **R4.2 Candidate eligibility** - pure deterministic exclusions with
  golden fixtures for missing auth, unsupported permissions, stale quota,
  unknown quota, cooldown, and model capability.
- historical-planned-code: **R4.3 Quota-aware ranking** - reset-aware ranking and reserve policy;
  prove it does not merely choose the largest remaining percentage.
- historical-planned-code: **R4.4 Persisted route decision** - one decision per attempt boundary,
  replay stability, operator pin behavior, and redacted explanation output.
- historical-planned-code: **R4.5 Successor attempt policy** - stop/checkpoint/join before legal
...
- historical-planned-doc: **R5.1 Runtime/process contract** - process tree ownership, start,
  observe, signal, stop, join, recovery evidence, native sub-agent boundary, and
  interactive-dialog/stall classification.
- historical-planned-code: **R5.2 macOS process-tree runtime** - launch and observe one provider
  process group with bounded logs and no task semantics.
- historical-planned-code: **R5.3 Stop/join cleanup** - graceful deadline, force termination,
  descendant proof, descriptor/watcher cleanup, and zero-leftover integration
  tests.
- historical-planned-code: **R5.4 Resource policy and admission** - global/per-attempt worker,
  verifier, local-test, descendant, CPU, RSS, and time budgets with pure policy
  decisions and visible denial reasons.
- historical-planned-code: **R5.5 macOS resource sampling/enforcement** - process-tree CPU/RSS/
  descendant measurement, threshold actions, and real child-process fixtures.
- historical-planned-code: **R5.6 Progress evidence collectors** - bounded process, filesystem,
  Git, focused-test, PR, and CI observations without rendering or model prose.
- historical-planned-code: **R5.7 Receipt timer/store/rendering** - pulled forward into Gate A;
...
- historical-planned-code: **R5.8 No-progress and stall policy** - two-interval policy, bounded
  diagnostic, cancellation/detach, interactive prompt detection, and return of
  user control.
- historical-planned-code: **R5.9 Provider-native sub-agent containment** - aggregate descendant
...
- historical-planned-doc: **R6.1 `loopcoder run` operator contract** - command inputs, phases,
  prompts, outputs, side effects, stop/resume behavior, and human gates.
- historical-planned-code: **R6.2 Intake and preflight** - resolve project/issue, focused policy,
  Work Graph root, provider snapshot, route decision, and worktree claim without
  launching a provider on failure.
- historical-planned-code: **R6.3 One worker execution** - run one pinned attempt through Runtime
  and Supervisor, capture changes, and persist the outcome.
- historical-planned-code: **R6.4 LoopCoder-owned Git/PR delivery** - validate diff, commit, push,
  create one PR, and make replay return the existing PR.
- historical-planned-code: **R6.5 Zero-model CI/approval watcher** - event/cursor-based waiting,
  five-minute receipts, restart recovery, and no busy/model polling.
- historical-planned-code: **R6.6 Host-neutral attach/report stream** - pulled forward into
...
- historical-planned-code: **R6.7 Independent verifier and gate** - different eligible provider,
  read-only evidence, remote checks, structured verdict, and explicit merge
  authority.
- historical-planned-code: **R6.8 Direct-path end-to-end canary** - one docs fixture and one code
...
- historical-planned-doc: **R7.1 Workflow definition/materialization contract** - reusable steps,
  immutable materialized graph, owner approval, bounds, roles, dependencies,
  and final aggregation.
- historical-planned-code: **R7.2 Workflow compiler** - validate and materialize a small explicit
  plan into Work Graph rows with stable child identity.
- historical-planned-code: **R7.3 Ready-node scheduler and admission** - deterministic dependency
  gating, one active top-level worker by default, and shared resource admission.
- historical-planned-code: **R7.4 Workflow cancellation and restart** - parent/child cancellation,
  terminal replay, orphan classification, and no duplicate child launch.
- historical-planned-code: **R7.5 Child delivery isolation** - one writer/worktree/branch ownership
  policy and an explicit aggregation strategy that cannot create duplicate or
  conflicting PRs.
- historical-planned-code: **R7.6 Workflow observability** - parent/child progress and terminal
...
- historical-planned-doc: **R8.1 Compatibility/deprecation guide** - old command mapping, old/new
  state isolation, one-release warning period, v1.0 removal list, and operator
  recovery steps.
- historical-planned-code: **R8.2 v0.8 read-only importer/exporter** - import supported project and
  report history into new projections without mutating or deleting the source.
- historical-planned-code: **R8.3 Compatibility command shims** - explicit old-engine selection,
  warning output, and prevention of mixed writers for one work item.
- historical-planned-code: **R8.4 Provider/routing consolidation deletion** - remove superseded
  inventory, quota, availability, budget, usage, model, and route paths proven
  unused by the new direct path.
- historical-planned-code: **R8.5 Progress/wait/report consolidation deletion** - remove superseded
  progress-host, relay, outbox, wait-state, and duplicate lifecycle paths.
- historical-planned-code: **R8.6 State/nested/autonomous retirement** - retire state-branch leases
  and default nested/dispatch-wave/tick/trigger surfaces after compatibility and
  usage evidence.
- historical-planned-doc: **R8.7 Production operations guide** - install, first run, multi-repo,
  private repo, provider setup, quota interpretation, resource limits, reports,
  stop/recovery, upgrade, and uninstall.
- historical-planned-code: **R8.8 Release qualification** - macOS arm64 package/install/upgrade,
...
- docs: customer quickstart in install/version/init/skill-install/doctor/report
  ...
  boundary.
- historical-shipped-code: local `.loopcoder/` protection through `.git/info/exclude` in
  `init --repo` and `skill install --repo`.
- historical-shipped-code: first-run `init --repo/--gate`, defaulting new scaffolds to
  `adapters.gate: human-merge`.
- historical-shipped-code: `doctor --format text|json`, local-state checks, reportquery
  readability, installed-skill and hook checks.
- historical-shipped-code: `report --format json` compatibility array plus richer `records`
...
- historical-shipped-doc: spec — static model registry (per provider: models × depth tiers + defaults);
...
- historical-shipped-code: `internal/models` **leaf package** (static registry + pure validation + defaults;
  imports no orchestration/config/agent — they import it) + provider default model/depth.
- historical-shipped-code: `loopcoder models [--provider]` command — print models × depth × default per provider
...
- historical-shipped-code: parse-time validation of worker/verifier `model`+`reasoning_effort` vs registry (warn
  by default; `.delivery.yml`/CLI `--strict` escalates to reject).
- historical-shipped-code: `internal/agent/antigravity.go` agy runner (close stdin, `-p`, plain-text summary,
...
- historical-shipped-code: docs/reference — `loopcoder models` usage, model/depth config, agy setup + login.
...
- historical-shipped-doc: spec — rename map + **full consumer inventory** (grep: ~1068 refs / 60 files across
...
- historical-shipped-code: emit `[reporter]`; rename `internal/attestation`→`internal/reporter`,
...
- historical-shipped-code: **transition safety** — `relay_guard.go` (`ledgerHeaderRe`, `rolePattern`) **and
...
- historical-shipped-code: strengthen — model+depth display (`Gemini 3.1 Pro (High)`); **work-ID = the worker's
...
- historical-shipped-code: `loopcoder report` — on-demand, read-only command to list/query recent per-work reports
  (work-ID, role, model+depth, start/end, tokens) from the persisted log; this is how the
  reporter subsystem "reports at any time".
- historical-shipped-code: docs/reference — reporter concept + attestation→reporter prose in usage.md/worker.md
...
- historical-shipped-doc: spec — upgrade/migration plan (old-version detection, config-key migration map, log
  retention policy) + `loopcoder doctor` definition (who runs it, what it checks, which roles).
- historical-shipped-code: `loopcoder upgrade` 0.5.x→0.6.0 — detect old version; migrate any renamed config keys
...
- historical-shipped-code: old-file handling — config-key migration; **bounded cleanup of stale logs** (run logs,
  worktree-liveness, relay state) under a retention policy; leave `.attest` ledgers + schema
  untouched.
- historical-shipped-code: `loopcoder doctor` — **operator-run; default diagnoses only (read-only, safe anytime)**:
