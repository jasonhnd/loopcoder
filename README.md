...
auth readiness returns `unknown` with a reason. Network-declared auth probes,
including Antigravity's `agy models`, are recorded but not executed by default;
their readiness is `unknown` with `network-permission-denied`. The initial
schema intentionally has no `expires_at` field because no current adapter
exposes a credential-blind machine-readable expiry value.
