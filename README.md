...
`loopcoder doctor --repo .` is a read-only operational health command. It reports `[info]`, `[ok]`, `[warn]`, or `[fail]` checks for git, gh auth, `.delivery.yml`, Worker and Verifier provider CLIs, provider auth probes where stable, origin/default branch, selected binary version/track, `min_loopcoder_version`, model/depth validity, reporter/relay wiring, installed skill freshness, audit readiness, local-state exclude protection, tracked `.loopcoder/` files, reportquery readability, migration status, stale local state counts, and the resolved host invocation profile. `--format json` emits `repo_path`, `version`, `commit`, `date`, `exit_code`, `host_profile`, and ordered `checks[]` objects with `name`, `status`, `hard`, `message`, and `fix_command`. Host profiles are selected by `LOOPCODER_HOST`, `.delivery.yml host.profile`, detection, then generic fallback; see [`docs/reference/host-profiles.md`](docs/reference/host-profiles.md).
