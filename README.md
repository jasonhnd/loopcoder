...
marked estimated rather than exact provider quota. When the persisted ledger is
empty, doctor derives a read-only fallback from the same imported and file-based
report records shown by `loopcoder report`, and reports malformed imported
payloads as gap reasons.
