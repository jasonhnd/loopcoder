...
- `report --repo . --run <child>` renders the same reporter evidence as a
  compact receipt, while `--verbose` and `--format json` keep detailed and raw
  forms separate.
...
4. Observability evidence: `loopcoder status --repo . --format json` and
   ...
   tree; `loopcoder report --repo . --run <run-id>` gives a compact receipt
   suitable for chat review; local report records stay out of PR bodies, issue
