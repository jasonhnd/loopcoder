...
rows are absent it derives a bounded read-only view through the same local
report loader used by `loopcoder report`: imported SQLite report rows, current
run attempt/report JSON, pending relay records, and relay ledgers. Imported
report rows with malformed payload JSON are counted and surfaced as
`malformed-report-payloads:<n>` gap reasons instead of being silently ignored.
