...
| Gate | Decide whether pre-prod may promote to production | Effective config: `human-merge` or `auto`; empty legacy configs normalize to `auto` |
...
config, then the static model registry defaults. Runtime compatibility preserves
the existing behavior where an empty or missing promotion gate normalizes to
`auto`. New v0.6.1 scaffolds generated by `loopcoder init --repo .` write
`adapters.gate: human-merge`; use `loopcoder init --repo . --gate auto` or edit
`.delivery.yml` to opt into automatic production promotion.
...
Production promotion is the separate `promote` step. When the effective gate is
...
`adapters.gate: human-merge` keeps production promotion human-directed.
