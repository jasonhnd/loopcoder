...
Production promotion is the separate `promote` step. With an effective
`adapters.gate: auto` value, it auto-promotes only when required CI is green,
...
production to the recorded prior-stable SHA if post-promote checks fail. New
scaffolds generated by `loopcoder init --repo .` use
`adapters.gate: human-merge`, which requires the human-directed production
promotion path unless the project explicitly opts into `auto`.
...
When a schema-v9 database is opened, migration 10 records local backup metadata
before creating the v0.8 tables. Legacy v0.7 `runs` rows are imported only as
conservative DeliveryRun-compatible records; the migration does not synthesize
approvals, overrides, user intent, or plan fingerprints. Ambiguous non-terminal
legacy execution claims are marked `needs-human` with
