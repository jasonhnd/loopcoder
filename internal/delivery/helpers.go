...
func validateOverride(override Override) error {
	if override.SchemaVersion != SchemaOverride || override.ProjectID == "" || override.DeliveryRunID == "" || override.OverrideID == "" || override.OverrideKind == "" || override.AuthorizationFingerprint == "" || override.PolicyFingerprint == "" || override.PolicyDecisionID == "" || override.Justification == "" {
...
func wouldCreateCycle(ctx context.Context, tx storage.Tx, runID, fromTaskID, toTaskID string) bool {
	...
	for rows.Next() {
		var id string
		if rows.Scan(&id) == nil && id == fromTaskID {
			return true
		}
	}
	return false
...
func ensureCurrentPolicyFingerprint(ctx context.Context, tx storage.Tx, projectID, runID, auth, policy string) error {
	var storedPolicy string
	err := tx.QueryRow(ctx, `SELECT policy_fingerprint FROM delivery_runs WHERE project_id = ? AND delivery_run_id = ? AND authorization_fingerprint = ?`, projectID, runID, auth).Scan(&storedPolicy)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return typed(ErrStaleApprovalCode, "authorization fingerprint does not match current run")
		}
		return fmt.Errorf("inspect run policy fingerprint: %w", err)
	}
	if storedPolicy != policy {
		return typed(ErrStaleApprovalCode, "override policy fingerprint does not match current run")
	}
	return nil
}

func replay(ctx context.Context, tx storage.Tx, key, projectID, runID, op string, request any, out any) (bool, error) {
	key = idempotencyKey(key, op, request)
...
func idempotencyKey(key, op string, request any) string {
	...
	return "idem_" + digestBase32(op, string(bytes))[:32]
}
