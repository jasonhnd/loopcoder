...
func TestMultiWindowNormalize(t *testing.T) {
	...
	rem := 90.0
	lim := 100.0
	raw, _ := json.Marshal([]antigravityquota.RawWindow{
	...
	snap, err := antigravityquota.ParseJSONFixture(raw, antigravityquota.ParseOptions{Now: t0(), Source: "fixture"})
	if err != nil {
		t.Fatal(err)
	}
	...
	// five_hour finite
	var five antigravityquota.Window
	for _, w := range snap.Windows {
		if w.Kind == antigravityquota.WindowFiveHour {
			five = w
		}
	}
	if five.Limit.Class != antigravityquota.QtyFinite || five.ResetAt == nil {
		t.Fatalf("%+v", five)
	}
	...
	for _, w := range snap.Windows {
		if w.Kind == antigravityquota.WindowWeekly {
			if w.Remaining.Class != antigravityquota.QtyUnlimited {
				t.Fatalf("%+v", w.Remaining)
			}
			if antigravityquota.IsNumericZero(w.Remaining) {
				t.Fatal("unlimited as zero")
			}
		}
		if w.Kind == antigravityquota.WindowCredit {
			if w.Limit.Class != antigravityquota.QtyMissing {
				t.Fatal(w.Limit)
			}
			// must not fabricate limit
			if w.Diagnostic != "limit_missing_not_fabricated" && w.Limit.Class == antigravityquota.QtyFinite {
...
func TestMissingDistinctFromZero(t *testing.T) {
	...
	snap, err := antigravityquota.ParseJSONFixture(raw, antigravityquota.ParseOptions{Now: t0()})
	if err != nil {
		t.Fatal(err)
	}
	var miss, zero antigravityquota.Quantity
	for _, w := range snap.Windows {
		if w.Kind == antigravityquota.WindowFiveHour {
			miss = w.Remaining
		}
	...
	if miss.Class != antigravityquota.QtyMissing || antigravityquota.IsNumericZero(miss) {
		t.Fatalf("missing=%+v", miss)
	}
	if zero.Class != antigravityquota.QtyZero || !antigravityquota.IsNumericZero(zero) {
		t.Fatalf("zero=%+v", zero)
	}
}

func TestMalformedAndUnavailable(t *testing.T) {
	_, err := antigravityquota.ParseJSONFixture([]byte("not-json"), antigravityquota.ParseOptions{Now: t0()})
	if err == nil {
		t.Fatal("expected malformed")
	}
	snap, err := antigravityquota.ParseJSONFixture([]byte("[]"), antigravityquota.ParseOptions{Now: t0(), ForceStatus: "unavailable"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestResetTimezoneAndBoundary(t *testing.T) {
	...
	if w.Remaining.Class != antigravityquota.QtyUnknown {
		t.Fatal(w.Remaining)
	}
	// bare small number rejected
	raw2, _ := json.Marshal([]antigravityquota.RawWindow{
		{Kind: "weekly", Scope: "account", ResetRFC3339: "12345"},
	})
	snap2, _ := antigravityquota.ParseJSONFixture(raw2, antigravityquota.ParseOptions{Now: t0()})
