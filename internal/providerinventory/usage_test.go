...
func TestBuildQuotaUsageBudgetFromReportsSurfacesMalformedPayloads(t *testing.T) {
	ctx := context.Background()
	now := fixedInventoryNow()
	store, err := storage.Open(ctx, storage.Options{Path: filepath.Join(t.TempDir(), "loopcoder.db"), Now: fixedInventoryNow})
	if err != nil {
	...
	defer store.Close()
	if err := insertStoredReportForUsageTest(ctx, store, "report-good", "project-usage", "run-usage", validUsageReport(3, 4, 7), now); err != nil {
		t.Fatalf("insert good report: %v", err)
	}
	if err := store.WithWriteTx(ctx, func(tx storage.Tx) error {
		_, err := tx.Exec(ctx, `INSERT OR IGNORE INTO reports(id, project_id, run_id, role, provider, model, payload_json, created_at, source_path, source_hash, source_kind)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
			"report-bad", "project-usage", "run-usage", "worker", "codex", "gpt-fixture", "{", formatTime(now), ".loopcoder/runs/run/events.jsonl", "sha256:bad", "fixture")
		return err
	}); err != nil {
		t.Fatalf("insert malformed report: %v", err)
	}

	report, err := BuildQuotaUsageBudgetFromReports(ctx, store, "project-usage", now, true)
	if err != nil {
		t.Fatalf("BuildQuotaUsageBudgetFromReports: %v", err)
	}
	if !containsString(report.GapReasons, "malformed-report-payloads:1") {
		t.Fatalf("gap reasons = %#v, want malformed-report-payloads:1", report.GapReasons)
	}
	if len(filterUsageKind(report.UsageRecords, QuantityTotalTokens)) != 1 {
		t.Fatalf("usage records = %#v, want good report still derived", report.UsageRecords)
	}
	if err := RefreshUsageLedgerFromReports(ctx, store, "project-usage", now); err == nil || !strings.Contains(err.Error(), "usage ledger refresh: malformed-report-payloads:1") {
		t.Fatalf("RefreshUsageLedgerFromReports error = %v, want malformed payload count", err)
	}
}

func TestBuildQuotaUsageBudgetFromReportQueryRecordsUsesLiveReportShape(t *testing.T) {
	now := fixedInventoryNow()
	record := validUsageReport(11, 13, 24)
	record.WorkID = "job-live-shape"
	report, err := BuildQuotaUsageBudgetFromReportQueryRecords([]reportquery.Record{{
		Report: record,
		Source: "attempt",
		RunID:  "run-live-shape",
		Path:   filepath.Join(".loopcoder", "runs", "run-live-shape", "workers", "job-live-shape.attempt.json"),
	}}, "project-usage", now, true)
	if err != nil {
		t.Fatalf("BuildQuotaUsageBudgetFromReportQueryRecords: %v", err)
	}
	totals := filterUsageKind(report.UsageRecords, QuantityTotalTokens)
	if len(totals) != 1 || totals[0].Value != 24 || totals[0].DeliveryRunID != "run-live-shape" {
		t.Fatalf("total token records = %#v, want live report-derived total", totals)
	}
	if !containsString(report.GapReasons, "derived-from-local-reporter-files") {
		t.Fatalf("gap reasons = %#v, want local reporter source", report.GapReasons)
	}
}
