...
func inspectAuthReadiness(ctx context.Context, adapter AdapterDeclaration, candidate candidate, installationID string, now time.Time, deps Deps) ([]AccountProfile, []AuthReadiness, *ProbeResult) {
	...
	if len(adapter.AuthArtifactPaths) > 0 {
		return inspectAuthArtifacts(adapter, installationID, now, deps)
	}
	if len(adapter.AuthEnvironmentNames) > 0 {
		return inspectAuthEnvironment(adapter, installationID, now, deps)
	}
	readiness := unsupportedAuthReadiness(adapter, &installationID, now, "auth-readiness-unsupported")
...
func inspectAuthCommand(ctx context.Context, adapter AdapterDeclaration, candidate candidate, installationID string, now time.Time, deps Deps) ([]AccountProfile, []AuthReadiness, *ProbeResult) {
	...
	probe.ProbeCommandID = "auth-readiness"
	probe.ProbeMethod = ProbeMethodFixedCommand
	...
	probe.Argv = redactArgv(argv)
	probe.EnvironmentKeys = environmentKeys(env)
	probe.Source = SourceDescriptor{Kind: "command", AdapterID: adapter.AdapterID, ProbeCommandID: "auth-readiness", DiscoverySource: string(candidate.source), ExecutableName: filepath.Base(candidate.path)}
	...
	probe.setParsedFields(map[string]string{
		...
		"parser":        "allowlisted-text-auth-status",
	})
...
func classifyAuthLine(line string) (ReadinessState, AuthorizationScopeState, string, bool, []string, bool) {
	...
	case strings.Contains(lower, "expired") || strings.Contains(lower, "revoked") || strings.Contains(lower, "refresh required"):
		return ReadinessExpired, AuthorizationUnknown, "", true, []string{"auth-expired"}, true
	case strings.Contains(lower, "not authenticated") || strings.Contains(lower, "unauthenticated") || strings.Contains(lower, "logged out") || strings.Contains(lower, "not logged in") || strings.Contains(lower, "login required"):
		return ReadinessNotAuthenticated, AuthorizationNotApplicable, "", true, []string{"auth-not-authenticated"}, true
	case strings.Contains(lower, "partial") || strings.Contains(lower, "limited scope") || strings.Contains(lower, "limited authorization"):
		return ReadinessReady, AuthorizationPartial, "partial authorization reported by provider status", false, []string{"authorization-partial"}, true
