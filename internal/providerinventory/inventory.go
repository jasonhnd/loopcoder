...
func markInstallationStale(ctx context.Context, tx storage.Tx, id, nowText string) error {
	...
	_, err = tx.Exec(ctx, `UPDATE provider_installations
		SET record_version = record_version + 1,
			...
			confidence = 'stale',
			payload_json = ?
		WHERE provider_installation_id = ?`, nowText, string(nextPayload), id)
	return err
}
...
func (b *boundedBuffer) Write(p []byte) (int, error) {
	...
	if b.combined != nil {
		if combinedRemain := b.combined.remaining.Load(); combinedRemain < remain {
			remain = combinedRemain
		}
	}
	if remain <= 0 {
	...
	if b.combined != nil {
		b.combined.remaining.Add(-int64(len(chunk)))
	}
	return len(p), nil
}

func (b *boundedBuffer) String() string  { return b.data.String() }
func (b *boundedBuffer) Truncated() bool { return b.truncated.Load() }
...
func insertProbe(ctx context.Context, tx storage.Tx, probe ProbeResult) error {
	...
	_, err = tx.Exec(ctx, `INSERT INTO provider_probe_results(
		probe_result_id, schema_version, record_version, adapter_id, adapter_declaration_id,
		...
		secret_finding_count, captured_at, freshness_state, confidence,
		terminal_error_code, payload_json
	) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		probe.ProbeResultID, probe.SchemaVersion, probe.RecordVersion, probe.AdapterID,
		probe.AdapterDeclarationID, providerInstallationID, probe.ProbeKind, probe.ProbeCommandID,
		...
		probe.SecretFindingCount, probe.CapturedAt, string(probe.FreshnessState),
...
func upsertInstallation(ctx context.Context, tx storage.Tx, installation ProviderInstallation) error {
	...
	knownLimitations, _ := json.Marshal(installation.KnownLimitations)
	gaps, _ := json.Marshal(installation.GapReasons)
	...
	ON CONFLICT(provider_installation_id) DO UPDATE SET
		...
		installation.ExecutableName, mustJSON(installation.ExecutableIdentity),
		...
		string(knownLimitations), installation.CreatedAt, installation.UpdatedAt, installation.CapturedAt,
		installation.StaleAfter, string(installation.FreshnessState), string(installation.Confidence),
		installation.SideEffectClass, installation.Classification, mustJSON(installation.Source),
		mustJSON(installation.Evidence), string(gaps), installation.TerminalErrorCode, string(payload))
	return err
}
