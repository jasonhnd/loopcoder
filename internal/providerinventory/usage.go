...
func RefreshUsageLedgerFromReports(ctx context.Context, store storage.Store, projectID string, now time.Time) error {
	...
	return store.WithWriteTx(ctx, func(tx storage.Tx) error {
		records, err := loadStoredReports(ctx, tx, projectID)
		if err != nil {
			return err
		}
		for _, reportRecord := range records {
...
func BuildQuotaUsageBudgetFromReports(ctx context.Context, store storage.Store, projectID string, now time.Time, includeRecords bool) (QuotaUsageBudgetReport, error) {
	...
	var usageRecords []UsageRecord
	err := store.WithTx(ctx, func(tx storage.Tx) error {
		reports, err := loadStoredReports(ctx, tx, projectID)
		if err != nil {
			return err
		}
		for _, reportRecord := range reports {
	...
	report := QuotaUsageBudgetReport{
		...
		AvailabilityScores: []any{},
		CircuitBreakers:    []any{},
		GapReasons:         dedupeStrings(append(usageReportGaps(usageRecords, nil), "derived-from-imported-reporter-records")),
	}
	if includeRecords {
		report.UsageRecords = usageRecords
	}
...
func loadStoredReports(ctx context.Context, tx storage.Tx, projectID string) ([]StoredReportRecord, error) {
