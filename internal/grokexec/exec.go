...
const (
	AdapterID     = "grok"
	SchemaCommand = "loopcoder.grok.exec.command.v1"
)

// ErrAuth is a typed auth refusal for Grok execution.
var ErrAuth = errors.New("grokexec: auth")
...
func DefaultCaps() Caps {
	return Caps{
		...
		Models: []grokobs.ModelRecord{
			{CanonicalID: "grok-4", Aliases: []string{"g4"}, Efforts: []string{"low", "medium", "high"}},
			{CanonicalID: "grok-3", Efforts: []string{"medium", "high"}},
		},
	}
}
...
func (p *Planner) Plan(req providerexec.Request) (CommandPlan, error) {
	...
	if !p.Caps.Installed {
		return CommandPlan{}, fmt.Errorf("%w: grok not installed", providerexec.ErrUnsupported)
	}
	if p.Caps.Auth == "missing" {
		return CommandPlan{}, fmt.Errorf("%w: auth missing", ErrAuth)
	}
	model := req.Route.Model
	can, ok, _ := grokobs.NormalizeAlias(p.Caps.Models, model)
	if !ok {
	...
	return CommandPlan{
		Schema: SchemaCommand, Binary: "grok", Args: args,
...
// Adapter implements providerexec.Adapter for Grok.
type Adapter struct {
	Planner Planner
...
func (a *Adapter) Execute(ctx context.Context, req providerexec.Request) (providerexec.Outcome, error) {
	...
	return providerexec.Outcome{
		...
		Process:    providerexec.ProcessEvidence{Adapter: AdapterID, Version: "v1", Command: "grok exec"},
		ExitCode:   0,
...
func ScrubEnv(env []string) []string {
	...
	for _, e := range env {
		...
		if uk == "GROK_MODEL" || uk == "GROK_PERMISSION" || uk == "GROK_EFFORT" ||
			uk == "GROK_NATIVE_DELEGATION" || uk == "OPENAI_MODEL" {
			continue
		}
...
func ImmutableRetry(a *Adapter, req providerexec.Request) (CommandPlan, CommandPlan, error) {
	...
	mutated := frozen
	mutated.Models = append(append([]grokobs.ModelRecord{}, frozen.Models...), grokobs.ModelRecord{CanonicalID: "new-model"})
