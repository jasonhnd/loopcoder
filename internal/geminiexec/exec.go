...
const (
	AdapterID     = "gemini"
	SchemaCommand = "loopcoder.gemini.exec.command.v1"
)

// ErrAuth is a typed auth refusal for Gemini CLI execution.
var ErrAuth = errors.New("geminiexec: auth")
...
func DefaultCaps() Caps {
	return Caps{
		...
		Models: []geminiobs.ModelRecord{
			{CanonicalID: "gemini-2.5-flash", Aliases: []string{"flash"}, Efforts: []string{"low", "medium", "high"}},
			{CanonicalID: "gemini-2.5-pro", Efforts: []string{"medium", "high"}},
		},
	}
}
...
func (p *Planner) Plan(req providerexec.Request) (CommandPlan, error) {
	...
	if !p.Caps.Installed {
		return CommandPlan{}, fmt.Errorf("%w: gemini not installed", providerexec.ErrUnsupported)
	}
	if p.Caps.Auth == "missing" {
		return CommandPlan{}, fmt.Errorf("%w: auth missing", ErrAuth)
	}
	model := req.Route.Model
	can, ok, _ := geminiobs.NormalizeAlias(p.Caps.Models, model)
	if !ok {
	...
	return CommandPlan{
		Schema: SchemaCommand, Binary: "gemini", Args: args,
...
// Adapter implements providerexec.Adapter for Gemini CLI.
type Adapter struct {
	Planner Planner
...
func (a *Adapter) Execute(ctx context.Context, req providerexec.Request) (providerexec.Outcome, error) {
	...
	return providerexec.Outcome{
		...
		Process:    providerexec.ProcessEvidence{Adapter: AdapterID, Version: "v1", Command: "gemini exec"},
		ExitCode:   0,
...
func ScrubEnv(env []string) []string {
	...
	for _, e := range env {
		...
		if uk == "GEMINI_MODEL" || uk == "GEMINI_PERMISSION" || uk == "GEMINI_EFFORT" ||
			uk == "GEMINI_NATIVE_DELEGATION" || uk == "OPENAI_MODEL" {
			continue
		}
...
func ImmutableRetry(a *Adapter, req providerexec.Request) (CommandPlan, CommandPlan, error) {
	...
	mutated := frozen
	mutated.Models = append(append([]geminiobs.ModelRecord{}, frozen.Models...), geminiobs.ModelRecord{CanonicalID: "new-model"})
