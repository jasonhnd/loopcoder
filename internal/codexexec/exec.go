...
const (
	AdapterID     = "codex"
	SchemaCommand = "loopcoder.codex.exec.command.v1"
)

// ErrAuth is a typed auth refusal for codex execution.
var ErrAuth = errors.New("codexexec: auth")
...
func DefaultCaps() Caps {
	return Caps{
		...
		Models: []codexobs.ModelRecord{
			{CanonicalID: "o4-mini", Aliases: []string{"mini"}, Efforts: []string{"low", "medium", "high"}},
			{CanonicalID: "gpt-5", Efforts: []string{"medium", "high"}},
		},
	}
}
...
func (p *Planner) Plan(req providerexec.Request) (CommandPlan, error) {
	...
	if !p.Caps.Installed {
		return CommandPlan{}, fmt.Errorf("%w: codex not installed", providerexec.ErrUnsupported)
	}
	if p.Caps.Auth == "missing" {
		return CommandPlan{}, fmt.Errorf("%w: auth missing", ErrAuth)
	}
	model := req.Route.Model
	can, ok, _ := codexobs.NormalizeAlias(p.Caps.Models, model)
	if !ok {
	...
	return CommandPlan{
		Schema: SchemaCommand, Binary: "codex", Args: args,
...
// Adapter implements providerexec.Adapter for Codex.
type Adapter struct {
	Planner Planner
...
func (a *Adapter) Execute(ctx context.Context, req providerexec.Request) (providerexec.Outcome, error) {
	...
	// Requested provider must be codex when set to something else
	if req.Route.Provider != "" && req.Route.Provider != AdapterID {
	...
	return providerexec.Outcome{
		...
		Process:    providerexec.ProcessEvidence{Adapter: AdapterID, Version: "v1", Command: "codex exec"},
		ExitCode:   0,
...
func ScrubEnv(env []string) []string {
	...
	for _, e := range env {
		...
		if uk == "CODEX_MODEL" || uk == "CODEX_PERMISSION" || uk == "CODEX_EFFORT" ||
			uk == "CODEX_NATIVE_DELEGATION" || uk == "OPENAI_MODEL" {
			continue
		}
...
func ImmutableRetry(a *Adapter, req providerexec.Request) (CommandPlan, CommandPlan, error) {
	...
	mutated := frozen
	mutated.Models = append(append([]codexobs.ModelRecord{}, frozen.Models...), codexobs.ModelRecord{CanonicalID: "new-model"})
