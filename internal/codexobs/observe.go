...
const (
	AdapterID      = "codex"
	SchemaCatalog  = "loopcoder.codex.catalog.v1"
	SchemaSnapshot = "loopcoder.codex.obs.v1"
)

// ProbeInputs are fixture/injected local probe outputs (no real process).
type ProbeInputs struct {
	// ExecutablePresent is whether a codex binary would be found.
...
// Snapshot is the Codex observation aggregate (no credentials).
...
// Observer runs bounded Codex observation plans.
type Observer struct {
	// Last is preserved across failed probes.
	Last *Snapshot
}

// Descriptor returns the codex adapter registration document.
func Descriptor() providerdesc.Descriptor {
	return providerdesc.Descriptor{
		...
		Version: providerdesc.DescriptorVersion, DisplayName: "Codex CLI",
		Identity: providerdesc.Identity{InstallID: "codex-cli", Present: false},
		...
		ProbePlans: []providerdesc.ProbePlan{
			{Name: "lookpath_codex", Timeout: 2 * time.Second},
			{Name: "codex_version", Timeout: 3 * time.Second},
			{Name: "auth_status_local", Timeout: 3 * time.Second, Optional: true},
			{Name: "model_catalog_local", Timeout: 5 * time.Second},
		},
		Notes: "observation only; invoke is V090-103; quota is V090-041",
	}
}
...
func (o *Observer) Observe(in ProbeInputs) (Snapshot, error) {
	...
	runner := func(st obsplan.SourceStep) (obsplan.StepOutcome, map[string]string, string, string) {
		...
		if in.UnsupportedVersion && st.Name == "cli_primary" {
			return obsplan.OutcomeUnsupported, nil, "E_VERSION", "unsupported codex version"
		}
...
func (a *AsAdapter) Descriptor() providerdesc.Descriptor {
	...
	if a.In.ExecutablePresent {
		d.Models = []providerdesc.ModelEntry{{ID: "o4-mini", DisplayName: "o4-mini"}}
	} else {
		d.Models = nil
...
func (a *AsAdapter) Observe(op providerdesc.Operation, _ map[string]string) (providerdesc.Observation, error) {
	...
	obs := providerdesc.Observation{
		...
		Provenance: providerdesc.Provenance{Source: "codexobs_fixture", ObservedAt: time.Now().UTC(), Freshness: "fresh"},
