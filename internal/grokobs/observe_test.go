package grokobs_test

import (
	"testing"
	"time"

	"github.com/jasonhnd/loopcoder/internal/grokobs"
...
func TestInstalledCatalog(t *testing.T) {
	o := &grokobs.Observer{}
	snap, err := o.Observe(grokobs.ProbeInputs{
		ExecutablePresent: true, Version: "0.2.0", AuthState: "known",
		AccountProfile: "fixture-profile",
		RawModels: []string{
			"grok-4|g4,g4-fast|200000|low,medium,high",
			"grok-3|g3|400000|medium",
		},
		Now: t0,
	})
	if err != nil {
	...
	// alias normalize
	can, ok, expl := grokobs.NormalizeAlias(snap.Models, "g4")
	if !ok || can != "grok-4" {
		t.Fatalf("%s %v %s", can, ok, expl)
	}
	// unknown not defaulted
	_, ok, expl = grokobs.NormalizeAlias(snap.Models, "auto")
	if ok {
		t.Fatalf("auto should not resolve: %s", expl)
	}
	_, ok, _ = grokobs.NormalizeAlias(snap.Models, "totally-unknown")
	if ok {
...
func TestNotInstalled(t *testing.T) {
	o := &grokobs.Observer{}
	snap, err := o.Observe(grokobs.ProbeInputs{ExecutablePresent: false, AuthState: "missing", Now: t0})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPreserveLastOnTimeout(t *testing.T) {
	o := &grokobs.Observer{}
	first, err := o.Observe(grokobs.ProbeInputs{
		ExecutablePresent: true, Version: "1.0", AuthState: "known",
		RawModels: []string{"grok-4||1000|low"}, Now: t0,
	})
	if err != nil {
		t.Fatal(err)
	}
	second, err := o.Observe(grokobs.ProbeInputs{
...
func TestDefaultAliasRejected(t *testing.T) {
	models := []grokobs.ModelRecord{{CanonicalID: "grok-4", Aliases: []string{"g4"}}}
	for _, bad := range []string{"default", "auto", ""} {
		_, ok, _ := grokobs.NormalizeAlias(models, bad)
		if ok {
...
func TestRegistryConformance(t *testing.T) {
	...
	ad := &grokobs.AsAdapter{Obs: &grokobs.Observer{}, In: grokobs.ProbeInputs{
		ExecutablePresent: true, Version: "0.1", AuthState: "known",
		RawModels: []string{"grok-4|flash|1|low"}, Now: t0,
	}}
	...
	for _, op := range []providerdesc.Operation{providerdesc.OpDiscover, providerdesc.OpAuthStatus, providerdesc.OpCatalog} {
		obs, err := reg.Observe(grokobs.AdapterID, op, nil)
		if err != nil || !obs.OK {
			t.Fatalf("%s %+v err=%v", op, obs, err)
		}
	}
	_, err := reg.Observe(grokobs.AdapterID, providerdesc.OpInvoke, nil)
	if err == nil {
