...
const (
	AdapterID      = "grok"
	SchemaCatalog  = "loopcoder.grok.catalog.v1"
	SchemaSnapshot = "loopcoder.grok.obs.v1"
)
...
// Snapshot is the Grok observation aggregate (no credentials).
...
// Observer runs bounded Grok observation plans.
...
func Descriptor() providerdesc.Descriptor {
	return providerdesc.Descriptor{
		...
		Version: providerdesc.DescriptorVersion, DisplayName: "Grok CLI",
		Identity: providerdesc.Identity{InstallID: "grok-cli", Present: false},
		...
		ProbePlans: []providerdesc.ProbePlan{
			{Name: "lookpath_grok", Timeout: 2 * time.Second},
			{Name: "grok_version", Timeout: 3 * time.Second},
...
func (o *Observer) Observe(in ProbeInputs) (Snapshot, error) {
	...
	runner := func(st obsplan.SourceStep) (obsplan.StepOutcome, map[string]string, string, string) {
		...
		if in.UnsupportedVersion && st.Name == "cli_primary" {
			return obsplan.OutcomeUnsupported, nil, "E_VERSION", "unsupported grok version"
		}
...
func (a *AsAdapter) Descriptor() providerdesc.Descriptor {
	...
	if a.In.ExecutablePresent {
		d.Models = []providerdesc.ModelEntry{{ID: "grok-4", DisplayName: "grok-4"}}
	} else {
		d.Models = nil
...
func (a *AsAdapter) Observe(op providerdesc.Operation, _ map[string]string) (providerdesc.Observation, error) {
	...
	obs := providerdesc.Observation{
		...
		Provenance: providerdesc.Provenance{Source: "grokobs_fixture", ObservedAt: time.Now().UTC(), Freshness: "fresh"},
