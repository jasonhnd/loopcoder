...
func TestMultiWindowNormalize(t *testing.T) {
	...
	rem := 90.0
	lim := 100.0
	raw, _ := json.Marshal([]claudequota.RawWindow{
	...
	snap, err := claudequota.ParseJSONFixture(raw, claudequota.ParseOptions{Now: t0(), Source: "fixture"})
	if err != nil {
		t.Fatal(err)
	}
	...
	// five_hour finite
	var five claudequota.Window
	for _, w := range snap.Windows {
		if w.Kind == claudequota.WindowFiveHour {
			five = w
		}
	}
	if five.Limit.Class != claudequota.QtyFinite || five.ResetAt == nil {
		t.Fatalf("%+v", five)
	}
	...
	for _, w := range snap.Windows {
		if w.Kind == claudequota.WindowWeekly {
			if w.Remaining.Class != claudequota.QtyUnlimited {
				t.Fatalf("%+v", w.Remaining)
			}
			if claudequota.IsNumericZero(w.Remaining) {
				t.Fatal("unlimited as zero")
			}
		}
		if w.Kind == claudequota.WindowCredit {
			if w.Limit.Class != claudequota.QtyMissing {
				t.Fatal(w.Limit)
			}
			// must not fabricate limit
			if w.Diagnostic != "limit_missing_not_fabricated" && w.Limit.Class == claudequota.QtyFinite {
...
func TestMissingDistinctFromZero(t *testing.T) {
	...
	snap, err := claudequota.ParseJSONFixture(raw, claudequota.ParseOptions{Now: t0()})
	if err != nil {
		t.Fatal(err)
	}
	var miss, zero claudequota.Quantity
	for _, w := range snap.Windows {
		if w.Kind == claudequota.WindowFiveHour {
			miss = w.Remaining
		}
	...
	if miss.Class != claudequota.QtyMissing || claudequota.IsNumericZero(miss) {
		t.Fatalf("missing=%+v", miss)
	}
	if zero.Class != claudequota.QtyZero || !claudequota.IsNumericZero(zero) {
		t.Fatalf("zero=%+v", zero)
	}
}

func TestMalformedAndUnavailable(t *testing.T) {
	_, err := claudequota.ParseJSONFixture([]byte("not-json"), claudequota.ParseOptions{Now: t0()})
	if err == nil {
		t.Fatal("expected malformed")
	}
	snap, err := claudequota.ParseJSONFixture([]byte("[]"), claudequota.ParseOptions{Now: t0(), ForceStatus: "unavailable"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestResetTimezoneAndBoundary(t *testing.T) {
	...
	if w.Remaining.Class != claudequota.QtyUnknown {
		t.Fatal(w.Remaining)
	}
	// bare small number rejected
	raw2, _ := json.Marshal([]claudequota.RawWindow{
		{Kind: "weekly", Scope: "account", ResetRFC3339: "12345"},
	})
	snap2, _ := claudequota.ParseJSONFixture(raw2, claudequota.ParseOptions{Now: t0()})
