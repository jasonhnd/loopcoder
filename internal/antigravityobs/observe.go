...
const (
	...
	SchemaCatalog  = "loopcoder.antigravity.catalog.v1"
	SchemaSnapshot = "loopcoder.antigravity.obs.v1"
)
...
// Snapshot is the Antigravity observation aggregate (no credentials).
...
// Observer runs bounded Antigravity observation plans.
...
func Descriptor() providerdesc.Descriptor {
	return providerdesc.Descriptor{
		...
		Version: providerdesc.DescriptorVersion, DisplayName: "Antigravity CLI",
		Identity: providerdesc.Identity{InstallID: "antigravity-cli", Present: false},
		...
		ProbePlans: []providerdesc.ProbePlan{
			{Name: "lookpath_antigravity", Timeout: 2 * time.Second},
			{Name: "antigravity_version", Timeout: 3 * time.Second},
			{Name: "auth_status_local", Timeout: 3 * time.Second, Optional: true},
			{Name: "model_catalog_local", Timeout: 5 * time.Second},
		},
		Notes: "separate from Gemini CLI; own install/account/catalog; observation only",
	}
}
...
func (o *Observer) Observe(in ProbeInputs) (Snapshot, error) {
	...
	runner := func(st obsplan.SourceStep) (obsplan.StepOutcome, map[string]string, string, string) {
		...
		if in.UnsupportedVersion && st.Name == "cli_primary" {
			return obsplan.OutcomeUnsupported, nil, "E_VERSION", "unsupported antigravity version"
		}
...
func (a *AsAdapter) Descriptor() providerdesc.Descriptor {
	...
	if a.In.ExecutablePresent {
		d.Models = []providerdesc.ModelEntry{{ID: "antigravity-flash", DisplayName: "antigravity-flash"}}
	} else {
		d.Models = nil
...
func (a *AsAdapter) Observe(op providerdesc.Operation, _ map[string]string) (providerdesc.Observation, error) {
	...
	obs := providerdesc.Observation{
		...
		Provenance: providerdesc.Provenance{Source: "antigravityobs_fixture", ObservedAt: time.Now().UTC(), Freshness: "fresh"},
