package geminiobs_test

import (
	"testing"
	"time"

	"github.com/jasonhnd/loopcoder/internal/geminiobs"
...
func TestInstalledCatalog(t *testing.T) {
	o := &geminiobs.Observer{}
	snap, err := o.Observe(geminiobs.ProbeInputs{
		ExecutablePresent: true, Version: "0.2.0", AuthState: "known",
		AccountProfile: "fixture-profile",
		RawModels: []string{
			"gemini-2.5-flash|flash,flash-lite|200000|low,medium,high",
			"gemini-2.5-pro|pro|400000|medium",
		},
		Now: t0,
	})
	if err != nil {
	...
	// alias normalize
	can, ok, expl := geminiobs.NormalizeAlias(snap.Models, "flash")
	if !ok || can != "gemini-2.5-flash" {
		t.Fatalf("%s %v %s", can, ok, expl)
	}
	// unknown not defaulted
	_, ok, expl = geminiobs.NormalizeAlias(snap.Models, "auto")
	if ok {
		t.Fatalf("auto should not resolve: %s", expl)
	}
	_, ok, _ = geminiobs.NormalizeAlias(snap.Models, "totally-unknown")
	if ok {
...
func TestNotInstalled(t *testing.T) {
	o := &geminiobs.Observer{}
	snap, err := o.Observe(geminiobs.ProbeInputs{ExecutablePresent: false, AuthState: "missing", Now: t0})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPreserveLastOnTimeout(t *testing.T) {
	o := &geminiobs.Observer{}
	first, err := o.Observe(geminiobs.ProbeInputs{
		ExecutablePresent: true, Version: "1.0", AuthState: "known",
		RawModels: []string{"gemini-2.5-flash||1000|low"}, Now: t0,
	})
	if err != nil {
		t.Fatal(err)
	}
	second, err := o.Observe(geminiobs.ProbeInputs{
...
func TestDefaultAliasRejected(t *testing.T) {
	models := []geminiobs.ModelRecord{{CanonicalID: "gemini-2.5-flash", Aliases: []string{"flash"}}}
	for _, bad := range []string{"default", "auto", ""} {
		_, ok, _ := geminiobs.NormalizeAlias(models, bad)
		if ok {
...
func TestRegistryConformance(t *testing.T) {
	...
	ad := &geminiobs.AsAdapter{Obs: &geminiobs.Observer{}, In: geminiobs.ProbeInputs{
		ExecutablePresent: true, Version: "0.1", AuthState: "known",
		RawModels: []string{"gemini-2.5-flash|flash|1|low"}, Now: t0,
	}}
	...
	for _, op := range []providerdesc.Operation{providerdesc.OpDiscover, providerdesc.OpAuthStatus, providerdesc.OpCatalog} {
		obs, err := reg.Observe(geminiobs.AdapterID, op, nil)
		if err != nil || !obs.OK {
			t.Fatalf("%s %+v err=%v", op, obs, err)
		}
	}
	_, err := reg.Observe(geminiobs.AdapterID, providerdesc.OpInvoke, nil)
	if err == nil {
