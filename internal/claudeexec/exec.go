...
const (
	AdapterID     = "claude"
	SchemaCommand = "loopcoder.claude.exec.command.v1"
)

// ErrAuth is a typed auth refusal for Claude Code execution.
var ErrAuth = errors.New("claudeexec: auth")
...
func DefaultCaps() Caps {
	return Caps{
		...
		Models: []claudeobs.ModelRecord{
			{CanonicalID: "claude-sonnet-4", Aliases: []string{"sonnet"}, Efforts: []string{"low", "medium", "high"}},
			{CanonicalID: "claude-opus-4", Efforts: []string{"medium", "high"}},
		},
	}
}
...
func (p *Planner) Plan(req providerexec.Request) (CommandPlan, error) {
	...
	if !p.Caps.Installed {
		return CommandPlan{}, fmt.Errorf("%w: claude not installed", providerexec.ErrUnsupported)
	}
	if p.Caps.Auth == "missing" {
		return CommandPlan{}, fmt.Errorf("%w: auth missing", ErrAuth)
	}
	model := req.Route.Model
	can, ok, _ := claudeobs.NormalizeAlias(p.Caps.Models, model)
	if !ok {
	...
	return CommandPlan{
		Schema: SchemaCommand, Binary: "claude", Args: args,
...
// Adapter implements providerexec.Adapter for Claude Code.
type Adapter struct {
	Planner Planner
...
func (a *Adapter) Execute(ctx context.Context, req providerexec.Request) (providerexec.Outcome, error) {
	...
	return providerexec.Outcome{
		...
		Process:    providerexec.ProcessEvidence{Adapter: AdapterID, Version: "v1", Command: "claude exec"},
		ExitCode:   0,
...
func ScrubEnv(env []string) []string {
	...
	for _, e := range env {
		...
		if uk == "CLAUDE_MODEL" || uk == "CLAUDE_PERMISSION" || uk == "CLAUDE_EFFORT" ||
			uk == "CLAUDE_NATIVE_DELEGATION" || uk == "OPENAI_MODEL" {
			continue
		}
...
func ImmutableRetry(a *Adapter, req providerexec.Request) (CommandPlan, CommandPlan, error) {
	...
	mutated := frozen
	mutated.Models = append(append([]claudeobs.ModelRecord{}, frozen.Models...), claudeobs.ModelRecord{CanonicalID: "new-model"})
