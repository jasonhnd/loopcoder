...
const (
	AdapterID      = "claude"
	SchemaCatalog  = "loopcoder.claude.catalog.v1"
	SchemaSnapshot = "loopcoder.claude.obs.v1"
)

// ProbeInputs are fixture/injected local probe outputs (no real process).
type ProbeInputs struct {
	// ExecutablePresent is whether a claude binary would be found.
...
// Snapshot is the Claude Code observation aggregate (no credentials).
...
// Observer runs bounded Claude Code observation plans.
type Observer struct {
	// Last is preserved across failed probes.
	Last *Snapshot
}

// Descriptor returns the claude adapter registration document.
func Descriptor() providerdesc.Descriptor {
	return providerdesc.Descriptor{
		...
		Version: providerdesc.DescriptorVersion, DisplayName: "Claude Code CLI",
		Identity: providerdesc.Identity{InstallID: "claude-cli", Present: false},
		...
		ProbePlans: []providerdesc.ProbePlan{
			{Name: "lookpath_claude", Timeout: 2 * time.Second},
			{Name: "claude_version", Timeout: 3 * time.Second},
...
func (o *Observer) Observe(in ProbeInputs) (Snapshot, error) {
	...
	runner := func(st obsplan.SourceStep) (obsplan.StepOutcome, map[string]string, string, string) {
		...
		if in.UnsupportedVersion && st.Name == "cli_primary" {
			return obsplan.OutcomeUnsupported, nil, "E_VERSION", "unsupported claude version"
		}
...
func (a *AsAdapter) Descriptor() providerdesc.Descriptor {
	...
	if a.In.ExecutablePresent {
		d.Models = []providerdesc.ModelEntry{{ID: "claude-sonnet-4", DisplayName: "claude-sonnet-4"}}
	} else {
		d.Models = nil
...
func (a *AsAdapter) Observe(op providerdesc.Operation, _ map[string]string) (providerdesc.Observation, error) {
	...
	obs := providerdesc.Observation{
		...
		Provenance: providerdesc.Provenance{Source: "claudeobs_fixture", ObservedAt: time.Now().UTC(), Freshness: "fresh"},
